...
if os.path.isfile('./trackers_with_newline.txt') or os.path.exists('./trackers_with_newline.txt'):
	os.remove('./trackers_with_newline.txt')
...
strall = '\n'.join([str(elem) for elem in alllist])

with open('./trackers_with_newline.txt','w') as fd:
    fd.write(strall)
